...
```{r, echo=F, message=F, warning=F}
set.seed(1)
xgb.sim.cv = xgb.cv(data = as.matrix(df_nna_n[,1:942]), label = (as.numeric(df_nna_n$y)-1), 
                    max.depth = 6, eta = 1, nrounds = 100,
                    nfold = 5, early_stopping_rounds = 5,
                    objective = "binary:logistic", eval_metric = "logloss")
sel_rounds = xgb.sim.cv$best_iteration
xgb.sim2 = xgboost(data = as.matrix(df_nna_n[,1:942]), label = (as.numeric(df_nna_n$y)-1), 
                  max.depth = 6, eta = 1, nrounds = sel_rounds,
                  objective = "binary:logistic", eval_metric = "logloss")
pred.xgb.sim2 = predict(xgb.sim2, as.matrix(df_nna_n[,1:942]))
pred.xgb.sim2 = ifelse(pred.xgb.sim2 > 0.5, 1, 0)
table(df_nna_n$y, pred.xgb.sim2)
